...
function Table({ isHidden }) {
  ...
  return (
    <div className="div-order">
        ...
        <tbody className="table-products">
          {data.map(({ productId, quantity, products: { name, price } }, index) => {
            if (quantity) {
              return (
                <tr key={ productId }>
                  <td data-testid={ DATA_TESTID_NUMBER + index } className="id-products">
                    {index + 1}
                  </td>
                  <td data-testid={ DATA_TESTID_NAME + index } className="name-products">
                    {name}
                  </td>
                  <td
                  ...
                  <td
                    data-testid={ DATA_TESTID_PRICE + index }
                    className="price-products"
                  >
                    {Number(price).toFixed(2).replace(/\./, ',')}
                  </td>
                  <td
                    data-testid={ DATA_TESTID_TOTAL + index }
                    className="sub-total-products"
                  >
      ...
      <p data-testid={ DATA_TESTID_TOTAL_PRICE } className="total-price">
        {`Total: R$${data.reduce((a, b) => a + b.quantity * Number(b.products.price), 0).toFixed(2).replace(/\./, ',')}`}
      </p>
    </div>
  );
}
